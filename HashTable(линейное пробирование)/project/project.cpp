...
void loadFile(string fileName, hashTable& table, int hash(string))
{
	table.Clear();
	ifstream file(fileName);
	string word;
	while (file >> word)
		table.Push(word, hash);
	file.close();
}
...
begin:
	...
	loadFile(fileName, table, hash_3);
	system("cls");
main_menu:
	do
	{
	...
	case 5:
	{
		...
		loadFile(fileName, table, hash_1);
		cout << "hash 1 функция: " << table.Average() << endl;
		loadFile(fileName, table, hash_2);
		cout << "hash 2 функция: " << table.Average() << endl;
		loadFile(fileName, table, hash_3);
		cout << "hash 3 функция: " << table.Average() << endl;
		system("pause");
	}
		break;
	case 6:
